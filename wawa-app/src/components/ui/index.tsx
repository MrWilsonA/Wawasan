...
const BTN_VARIANT: Record<string, string> = {
  ...
  amber: 'bg-amber-300 text-ink border-amber-500 [--sh:var(--color-amber-500)] hover:bg-amber-200',
  secondary: 'bg-white text-ink border-sand [--sh:#ded7c6] hover:bg-cream',
  ghost: 'bg-transparent text-ink-soft border-transparent [--sh:transparent] shadow-none hover:bg-black/5',
}

const BTN_SIZE: Record<string, string> = {
  sm: 'text-[13px] px-3.5 py-2 rounded-xl',
  md: 'text-[15px] px-5 py-2.5 rounded-2xl',
  lg: 'text-[17px] px-7 py-3.5 rounded-2xl',
}
...
export function Button({
  ...
  return (
    <button
      {...rest}
      className={cx(
        'inline-flex items-center justify-center gap-2 font-display font-extrabold uppercase tracking-wide',
...
export function Card({
  className, children, tone = 'white', ...rest
}: HTMLAttributes<HTMLDivElement> & { tone?: 'white' | 'cream' | 'sand' }) {
  const tones = { white: 'bg-white', cream: 'bg-cream', sand: 'bg-shell' }
  return (
    <div
      {...rest}
      className={cx(
        'rounded-3xl border-2 border-sand p-5 shadow-[0_4px_0_0_#e8e1d0]',
        tones[tone],
        className,
      )}
    >
...
export function Chip({
  children, color = 'teal', size = 'md', className,
}: {
  children: ReactNode
  color?: 'teal' | 'amber' | 'coral' | 'leaf' | 'grape' | 'sky' | 'ink'
  size?: 'sm' | 'md'
  className?: string
}) {
  const map = {
...
export function Ring({
  value, max = 100, size = 84, stroke = 11, color = '#00a191', track = '#eae4d5', children,
}: {
...
export function Callout({
  ...
  kind?: 'tip' | 'warning' | 'concept' | 'contrast' | 'story' | 'formula'
  title?: string
  children: ReactNode
}) {
  const map = {
    tip: { border: 'border-teal-200', bg: 'bg-teal-50', icon: '💡', text: 'text-teal-700' },
    warning: { border: 'border-coral-200', bg: 'bg-coral-50', icon: '⚠️', text: 'text-coral-600' },
    concept: { border: 'border-sky-200', bg: 'bg-sky-50', icon: '🧠', text: 'text-sky-600' },
    contrast: { border: 'border-grape-200', bg: 'bg-grape-50', icon: '⚖️', text: 'text-grape-600' },
    story: { border: 'border-amber-200', bg: 'bg-amber-50', icon: '📜', text: 'text-amber-600' },
    formula: { border: 'border-sand', bg: 'bg-cream', icon: '🔑', text: 'text-ink-soft' },
  }
  const s = map[kind]
  return (
...
export function DataTable({
  ...
  return (
    <div className="overflow-x-auto rounded-2xl border-2 border-sand bg-white">
      ...
      {tabs.map((t) => {
        ...
        return (
          <button
            ...
            className={cx(
              'rounded-full border-2 font-display font-extrabold transition-colors',
          ...
          >
            {t.label}
            {t.count !== undefined ? (
              <span className={cx('ml-1.5 rounded-full px-1.5 py-0.5 text-[11px]', active ? 'bg-teal-700' : 'bg-shell')}>
                {t.count}
              </span>
            ) : null}
          </button>
...
}: { icon: ReactNode; value: ReactNode; label: string; color?: 'teal' | 'amber' | 'coral' | 'leaf' | 'grape' | 'sky' }) {
  const map = {
    teal: 'bg-teal-50 border-teal-200', amber: 'bg-amber-50 border-amber-200',
    coral: 'bg-coral-50 border-coral-200', leaf: 'bg-leaf-50 border-leaf-200',
    grape: 'bg-grape-50 border-grape-200', sky: 'bg-sky-50 border-sky-200',
  }
  return (
...
export function EmptyState({ title, body, action }: { title: string; body: string; action?: ReactNode }) {
  return (
    <div className="rounded-3xl border-2 border-dashed border-sand bg-white/60 px-6 py-12 text-center">
